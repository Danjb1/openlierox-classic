...
void Menu_LocalInitialize(void)
{
    ...
    // Minimap box
    tLX->cFont.Draw(tMenu->bmpBuffer, 310,240,0xffff,"Playing");
    //tLX->cFont.Draw(tMenu->bmpBuffer, 480,240,0xffff,"H");
    //tLX->cFont.Draw(tMenu->bmpBuffer, 515,240,0xffff,"T");
...
bool Menu_GameSettings_Frame(void)
{
	...
	char buf[64];
	sprintf(buf, "%d%%%%",l);		// 4 %'s because it gets parsed as a va_list twice
...
void Menu_WeaponsRestrictions(char *szMod)
{
	...
	// Setup the buffer
	DrawImageAdv(tMenu->bmpBuffer, tMenu->bmpMainBack_wob, 120,150,120,150, 400,300);
	Menu_DrawBox(tMenu->bmpBuffer, 120,150, 520,440);
...
// Returns whether of not we have finished with the weapons restrictions
bool Menu_WeaponsRestrictions_Frame(void)
{
	...
	if(ev) {
		...
		switch(ev->iControlID) {
            ...
            case wr_Reset:
                if( ev->iEventMsg == BTN_MOUSEUP ) {
                    cWpnRestList.resetVisible(cWpnGameScript);
                }
                break;
