...
int Menu_MessageBox(char *sTitle, char *sText, int type)
{
	...
	if(tLX->cFont.GetWidth(sText)+10 > w) {
		w = tLX->cFont.GetWidth(sText)+30;
		x = 320-w/2;
	}
	...
	tLX->cFont.DrawCentre(tMenu->bmpBuffer, cx, y+5, 0xffff, sTitle);
	tLX->cFont.DrawCentre(tMenu->bmpBuffer, cx, cy, 0xffff, sText);

	Menu_RedrawMouse(true);
    
    
    // This fixes a problem with the escape key sometimes not functioning, and preventing the game quiting
...
void Menu_SvrList_DrawInfo(char *szAddress)
{
    ...
    tLX->cFont.DrawCentre(tMenu->bmpBuffer, 320, 155, 0xffff, "Server Details");
    ...
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,200, 0xffff, "Server name:");
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,220, 0xffff, "Level name:");
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,240, 0xffff, "Mod name:");
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,260, 0xffff, "State:");
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,280, 0xffff, "Playing:");

    tLX->cFont.Draw(tMenu->bmpBuffer, 140,310, 0xffff, "Game Type:");
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,330, 0xffff, "Lives:");
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,350, 0xffff, "Max Kills:");
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,370, 0xffff, "Loading Times:");
    tLX->cFont.Draw(tMenu->bmpBuffer, 140,390, 0xffff, "Bonuses:");
    
    if(!bGotDetails) {
        tLX->cFont.Draw(tMenu->bmpBuffer, 270,200, MakeColour(200,100,100), "Unable to query server");
        return;
    }
    ...
    int x = 290;
    tLX->cFont.Draw(tMenu->bmpBuffer, x,200, 0xffff, szName);
    tLX->cFont.Draw(tMenu->bmpBuffer, x,220, 0xffff, szMapName);
    tLX->cFont.Draw(tMenu->bmpBuffer, x,240, 0xffff, szModName);
    tLX->cFont.Draw(tMenu->bmpBuffer, x,260, 0xffff, states[nState]);
    tLX->cFont.Draw(tMenu->bmpBuffer, x,280, 0xffff, "%d / %d", nNumPlayers, nMaxWorms);

    tLX->cFont.Draw(tMenu->bmpBuffer, x,310, 0xffff, gamemodes[nGameMode]);
    tLX->cFont.Draw(tMenu->bmpBuffer, x,330, 0xffff, "%d", nLives);
    tLX->cFont.Draw(tMenu->bmpBuffer, x,350, 0xffff, "%d", nMaxKills);
    tLX->cFont.Draw(tMenu->bmpBuffer, x,370, 0xffff, "%d", nLoadingTime);
    tLX->cFont.Draw(tMenu->bmpBuffer, x,390, 0xffff, "%s", nBonuses ? "on" : "off");
}