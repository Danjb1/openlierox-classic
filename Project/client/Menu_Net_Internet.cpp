...
int Menu_Net_NETInitialize(void)
{
	...
	cInternet.Add( new CListview(),								ServerList, 40, 160, 560, 260);
			...
			case ServerList:
                ...
                if( ev->iEventMsg == LV_RIGHTCLK ) {
					...
					if(result && addr[0]) {
                        ...
                        cInternet.SendMessage( PopupMenu, MNM_ADDITEM, 3, (DWORD)"Server details" );
                    }
                }
				break;
...
void Menu_Net_NETUpdateList(void)
{
	...
	while(!GetKeyboard()->KeyUp[SDLK_ESCAPE] && updateList) {
        ...
        if( SvrCount > 0 ) {
            DrawRectFill(tMenu->bmpScreen, 222,282, 222+((float)CurServer/(float)SvrCount)*200.0f, 299, MakeColour(0,136,250));
