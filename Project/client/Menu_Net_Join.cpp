...
/*
   Joining is broken into 3 parts:
   1) Choosing the players
   2) Joining the server
   3) If we succeeded in join, the lobby
*/
...
{
	iNetMode = net_join;
	iJoinMenu = join_players;
	strcpy(sJoinAddress, sAddress);	

	if(!Menu_Net_JoinInitializePlayers()) {
		// Error
		return false;
	}
...
void Menu_Net_JoinLobbyFrame(int mouse)
{
	...
	// Check if the comminucation link between us & server is still ok
    ...
    Menu_redrawBufferRect(360,65,  280, 200);
	...
	for(i=0; i<gl->nMaxWorms; i++, w++) {
		...
		switch(l->iType) {
			...
			// Open
			case LBY_OPEN:
				//tLX->cFont.Draw(tMenu->bmpScreen, 77, y, 0xffff, "------");
				break;

			// Closed
			case LBY_CLOSED:
				//tLX->cFont.Draw(tMenu->bmpScreen, 77, y, 0xffff, "Closed");
				break;
			...
			case LBY_USED:
				...
				tLX->cFont.Draw(tMenu->bmpScreen, x+55, y-2, 0xffff, w->getName());    
