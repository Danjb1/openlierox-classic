...
void DrawEntities(SDL_Surface *bmpDest, CViewport *v)
{
	...
	for(int e=0;e<MAX_ENTITIES;e++,ent++) {
		...
		switch(ent->iType) {
			...
			// Beam
			case ENT_BEAM:
				end = ent->vPos + ent->vVel*ent->iType2;
					...
					switch(ent->iType) {
						...
						case ENT_GIB:
							if(VectorLength(ent->vVel) > 160)
								...
								int x = (int)ent->vPos.GetX();
								int y = (int)ent->vPos.GetY();

								DrawImageAdv(map->GetImage(),ent->bmpSurf,(int)ent->iRotation*4,8,x-1,y-1,4,4);
							}
							break;
					}
				}
