    ...
    for( int i=0; i<psRandom->nNumObjects; i++ ) {
        if( psRandom->psObjects ) {
            ...
            switch(psRandom->psObjects[i].Type) {
                ...
                // Stone
                case OBJ_STONE:
                    PlaceStone(psRandom->psObjects[i].Size,CVec(psRandom->psObjects[i].X, psRandom->psObjects[i].Y));
                    break;

                // Misc
                case OBJ_MISC:
                    PlaceMisc(psRandom->psObjects[i].Size,CVec(psRandom->psObjects[i].X, psRandom->psObjects[i].Y));
                    break;

                // Hole                
                case OBJ_HOLE:
                    CarveHole(psRandom->psObjects[i].Size,CVec(psRandom->psObjects[i].X, psRandom->psObjects[i].Y));
                    break;
            }
        }
    }
...
void CMap::UpdateMiniMap(int force)
{
	...
	// Optimize: Get rid of putpixel/getpixel and use addressing
	...
	for(my=0,mmy=0;my<Height;my+=ystep,mmy++) {
		...
		for(mx=0,mmx=0;mx<Width;mx+=xstep,mmx++) {
			...
			PutPixel(bmpMiniMap,mmx,mmy,GetPixel(bmpImage,(int)mx,(int)my));
		}
	}
...
int CMap::Load(char *filename)
{
	...
	for(int i=0;i<numobj;i++) {
		...
		// Place the object
		if(o.Type == OBJ_STONE)
			PlaceStone(o.Size,CVec(o.X, o.Y));
		if(o.Type == OBJ_MISC)
			PlaceMisc(o.Size,CVec(o.X, o.Y));
	}
    ...
    /*for(y=0; y<nGridRows; y++) {
        for(x=0; x<nGridCols; x++) {
            ...
            if(GridFlags[y*nGridCols+x] == PX_EMPTY)
                DrawRectFill(bmpImage,x*nGridWidth,y*nGridHeight,x*nGridWidth+nGridWidth,y*nGridHeight+nGridHeight, MakeColour(0,0,128));
            if(GridFlags[y*nGridCols+x] == PX_DIRT)
                DrawRectFill(bmpImage,x*nGridWidth,y*nGridHeight,x*nGridWidth+nGridWidth,y*nGridHeight+nGridHeight, MakeColour(255,0,0));
            if(GridFlags[y*nGridCols+x] == PX_ROCK)
                DrawRectFill(bmpImage,x*nGridWidth,y*nGridHeight,x*nGridWidth+nGridWidth,y*nGridHeight+nGridHeight, MakeColour(128,128,128));
            DrawRect(bmpImage,x*nGridWidth,y*nGridHeight,x*nGridWidth+nGridWidth,y*nGridHeight+nGridHeight, 0);
